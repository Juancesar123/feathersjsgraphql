...
let moduleExports = function batchLoaderResolvers(app, options) {
  ...
  let returns = {
    ...
    Query: {
      ...
      // !<DEFAULT> code: query-Databuku
      // getDatabuku(query: JSON, params: JSON, key: JSON): Databuku
      getDatabuku(parent, args, content, ast) {
        const feathersParams = convertArgs(args, content, ast);
        return databuku.get(args.key, feathersParams).then(extractFirstItem);
      },
